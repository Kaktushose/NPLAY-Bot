...
public class XpBooster extends Item {

    public XpBooster(int price, String name, String description, long duration, int type) {
        super(price, name, description, duration, type, ItemType.XPBOOSTER);
    }
    ...
    public void buy(Bot bot, Member member) {
        ...
        botUser.setXpBoosterBuyTime(System.currentTimeMillis());
    ...
    public void onItemExpiration(Bot bot, Member member) {
        BotUser botUser = bot.getDatabase().getBotUser(member.getIdLong());
        if (System.currentTimeMillis() - botUser.getXpBoosterBuyTime() >= duration) {
            botUser.getItemStack().remove(ItemType.XPBOOSTER);
            member.getUser().openPrivateChannel().queue(privateChannel -> {
                privateChannel.sendMessage(new EmbedBuilder()
                        .setColor(Color.ORANGE)
                        .setTitle("Item abgelaufen!")
                        .setDescription("Dein " + name + " ist abgelaufen!")
                        .addField("Gekauft am:", new SimpleDateFormat("dd.MM.yyyy HH:mm").format(new Date(botUser.getXpBoosterBuyTime())), false)
                        .build()).queue(msg -> {
                    botUser.setXpBoosterBuyTime(0);
                    bot.getDatabase().setBotUser(botUser);
