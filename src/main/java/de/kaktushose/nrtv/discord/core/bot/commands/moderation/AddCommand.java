...
public class AddCommand extends Command {
    ...
    protected void onCommand(Member executor, Arguments args, TextChannel channel, Message message) {
        channel.sendMessage(new EmbedBuilder()
                ...
                .addField("Syntax:", "`" + bot.getPrefix() + "add <coins|xp|diamonds> <@Member|@Rolle|all> <amount>`", false)
    ...
    public void onAddCoins(Member executor, Arguments args, TextChannel channel, Message message) {
        ...
        args.getAsMemberList(1).forEach(member -> {
            ...
            botUser.setCoins(botUser.getCoins() + args.getAsInteger(2));
        ...
        channel.sendMessage(new EmbedBuilder()
                .setTitle("Erfolg!")
                .setDescription("Die Münzen von " + member + " wurden um " + args.getAsInteger(2) + " erhöht")
                .setColor(Color.GREEN)
                .build()).queue();
    }

    @SubCommand({"xp", MEMBER_GROUP, NUMBER})
    public void onAddXp(Member executor, Arguments args, TextChannel channel, Message message) {
        List<Member> updatedMember = new ArrayList<>();
        args.getAsMemberList(1).forEach(member -> {
            updatedMember.add(member);
            BotUser botUser = bot.getDatabase().getBotUser(member.getIdLong());
            botUser.setXp(botUser.getXp() + args.getAsInteger(2));
            bot.getDatabase().setBotUser(botUser);
        });
        ...
        channel.sendMessage(new EmbedBuilder()
                .setTitle("Erfolg!")
                .setDescription("Die XP von " + member + " wurden um " + args.getAsInteger(2) + " erhöht")
                .setColor(Color.GREEN)
                .build()).queue();
    }

    @SubCommand({"diamonds", MEMBER_GROUP, NUMBER})
    public void onAddDiamonds(Member executor, Arguments args, TextChannel channel, Message message) {
        List<Member> updatedMember = new ArrayList<>();
        args.getAsMemberList(1).forEach(member -> {
            updatedMember.add(member);
            BotUser botUser = bot.getDatabase().getBotUser(member.getIdLong());
            botUser.setDiamonds(botUser.getDiamonds() + args.getAsInteger(2));
            bot.getDatabase().setBotUser(botUser);
        });
        ...
        channel.sendMessage(new EmbedBuilder()
                .setTitle("Erfolg!")
                .setDescription("Die Diamanten von " + member + " wurden um " + args.getAsInteger(2) + " erhöht")
                .setColor(Color.GREEN)
                .build()).queue();
    }
    ...
    protected void onHelp(Member executor, TextChannel channel) {
        channel.sendMessage(new EmbedBuilder()
                ...
                .setDescription("Syntax: `" +  bot.getPrefix() + "add <coins|xp|diamonds> <@Member|@Rolle|all> <amount>`")
                .addField("Beschreibung:", "Fügt einer Usergruppe die angegebene Anzahl an XP, Münzen oder Diamanten hinzu.\n" +
